fetch("http://localhost:3000/api/teddies").then(response => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        let imgBear = createHtml("img", {"class": "mx-auto h-40 w-40 rounded-2xl xl:w-56 xl:h-56 object-cover", "src": data[i].imageUrl, "alt": "Photo de l'ours en peluche " + data[i].name}, blockBear);

        /* let blockDetails = document.createElement("div");
        blockDetails.classList.add("space-y-2");
        blockBear.appendChild(blockDetails); */
        let blockDetails = createHtml("div", {"class": "space-y-2"}, blockBear);
        ...
        let blockPriceName = createHtml("div", {"class": "text-lg space-y-1"}, blockDetails);

        /* let nameBear = document.createElement("H3");
        nameBear.classList.add("font-extrabold");
        blockPriceName.appendChild(nameBear); */
        let nameBear = createHtml("H3", {"class": "font-extrabold"}, blockPriceName);
        ...
        let priceBear = createHtml("p", {"class": "text-indigo-400 font-bold"}, blockPriceName);
